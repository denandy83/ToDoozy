...
export class LabelRepository {
  ...
  findAll(): Label[] {
    return this.db.prepare('SELECT * FROM labels ORDER BY order_index ASC').all() as Label[]
  }
  ...
  findProjectsUsingLabel(labelId: string): Array<{ project_id: string; project_name: string; task_count: number }> {
    return this.db
      ...
      .all(labelId, labelId) as Array<{ project_id: string; project_name: string; task_count: number }>
  }
