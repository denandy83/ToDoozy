...
function formatLocalTime(iso: string): string {
  // HH:MM:SS in the user's system timezone
...
function formatLocalDateTime(iso: string): string {
  // Full local date+time for copy/export (includes TZ offset)
...
function formatEntry(e: LogEntry): string {
  ...
  return `[${formatLocalDateTime(e.timestamp)}] ${e.level.toUpperCase()} ${e.category}: ${e.message}${ctx}`
}
    ...
    <div className="flex h-full flex-col">
      <div className="mb-4">
        ...
        <p className="mt-2 text-sm font-light text-muted">
          Live connection events (Realtime channels and online/offline). Cleared on app restart. Times shown in your system timezone.
        </p>
      </div>

      <div className="flex-1 select-text cursor-text overflow-y-auto rounded-lg border border-foreground/10 bg-foreground/[0.02] p-3 font-mono text-[11px] leading-relaxed">
        {entries.length === 0 ? (
          ...
          <ul className="space-y-1">
            {entries.map((e) => (
              <li key={e.id} className="flex gap-3">
                <span className="flex-shrink-0 text-muted/60">{formatLocalTime(e.timestamp)}</span>
