...
// The stored value is `encode(digest(key, 'sha256'), 'hex')` in Postgres. These
// vectors are the canonical SHA-256 hex digests, so a match here proves the
// edge function computes the exact string the backfill migration stored.
describe('hashApiKey (edge function)', () => {
